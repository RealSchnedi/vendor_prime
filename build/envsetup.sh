function __print_prime_functions_help() {
cat <<EOF
...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        echo "Found device"
        if (adb shell getprop com.prime.device | grep -q "$PRIME_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $PRIME_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function githubremote()
{
    ...
    local REMOTE=$(git config --get remote.prime.projectname)
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop org.prime.device | grep -q "$PRIME_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop com.prime.device | grep -q "$PRIME_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
